...
type UserServiceClient interface {
	...
	AddUsersVerbose(ctx context.Context, opts ...grpc.CallOption) (UserService_AddUsersVerboseClient, error)
}
...
func (c *userServiceClient) AddUsersVerbose(ctx context.Context, opts ...grpc.CallOption) (UserService_AddUsersVerboseClient, error) {
	stream, err := c.cc.NewStream(ctx, &UserService_ServiceDesc.Streams[2], "/pb.UserService/AddUsersVerbose", opts...)
	if err != nil {
		return nil, err
	}
	x := &userServiceAddUsersVerboseClient{stream}
	return x, nil
}

type UserService_AddUsersVerboseClient interface {
	Send(*User) error
	Recv() (*UserResultStream, error)
	grpc.ClientStream
}

type userServiceAddUsersVerboseClient struct {
	grpc.ClientStream
}

func (x *userServiceAddUsersVerboseClient) Send(m *User) error {
	return x.ClientStream.SendMsg(m)
}

func (x *userServiceAddUsersVerboseClient) Recv() (*UserResultStream, error) {
	m := new(UserResultStream)
	if err := x.ClientStream.RecvMsg(m); err != nil {
		return nil, err
	}
...
func (UnimplementedUserServiceServer) AddUsersVerbose(UserService_AddUsersVerboseServer) error {
	return status.Errorf(codes.Unimplemented, "method AddUsersVerbose not implemented")
}
...
func _UserService_AddUsersVerbose_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(UserServiceServer).AddUsersVerbose(&userServiceAddUsersVerboseServer{stream})
}

type UserService_AddUsersVerboseServer interface {
	Send(*UserResultStream) error
	Recv() (*User, error)
	grpc.ServerStream
}

type userServiceAddUsersVerboseServer struct {
	grpc.ServerStream
}

func (x *userServiceAddUsersVerboseServer) Send(m *UserResultStream) error {
	return x.ServerStream.SendMsg(m)
}

func (x *userServiceAddUsersVerboseServer) Recv() (*User, error) {
	m := new(User)
	if err := x.ServerStream.RecvMsg(m); err != nil {
		return nil, err
	}
...
var UserService_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		{
			StreamName:    "AddUsersVerbose",
			Handler:       _UserService_AddUsersVerbose_Handler,
			ServerStreams: true,
			ClientStreams: true,
		},
	},
